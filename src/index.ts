...
export const validateControllerFirmware = async (
  ...
  if (parsedResponse?.data?.version === version) {
    ipLog(ip, `Up to date - ${parsedResponse?.data?.version}`, { clear: true })
    return
  }

  ipLog(ip, `Current version - ${parsedResponse?.data?.version}`, {
    clear: true,
  })
  ...
  const chip = getDeviceChip(parsedResponse.data)
